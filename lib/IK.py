...
def calculate_IK(input_tar, MLP_output_base, a, d, alpha):
    ...
    #     print(abs(iiiii) - torch.tensor([1.0]))
    ...
    for i in nan_index:
        ...
        the_NANLOSS_of_illegal_solution_with_num_and_Nan = the_NANLOSS_of_illegal_solution_with_num_and_Nan + \
                                                           (abs(save_what_caused_Error2_as_Nan[i])- torch.tensor([1.0])) * 20
    if len(nan_index) == 8:
        GG = 0
        for iii in nan_index:
            # if abs(save_what_caused_Error2_as_Nan[iii]) < 2:
            GG = GG + (abs(save_what_caused_Error2_as_Nan[i]) - torch.tensor([1.0])) * 10
        GG = GG / 8
        angle_solution = GG
...
def calculate_IK_test(input_tar, MLP_output_base, a, d, alpha):
    ...
    #         IK_test_incorrect += 1
    #         angle_solution = t4[ii] * 100 - torch.tensor([0])
    #         # print("角5{}推出来了".format(ii + 1), angle_solution)
    ...
    #         IK_test_incorrect += 1
    #         angle_solution = t5[iii] * 100 - torch.tensor([0])
    #         # print("角2{}推出来了".format(iii + 1), angle_solution)
